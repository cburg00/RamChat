...
class RecursiveUrlLoader(BaseLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        but it will still work in the expected way, just not lazy.
        This is modified to include the root url in the retrieval. """
        try:
        ...
        root_document = Document(
            page_content=self.extractor(text),
            metadata=self._gen_metadata(text, self.url),
        )

        if self.use_async:
            results = asyncio.run(self._async_get_child_links_recursive(self.url))
            if results is None:
                return iter([root_document])
            else:
                return iter([root_document] + results)
        else:
            return iter([root_document] + list(self._get_child_links_recursive(self.url)))
